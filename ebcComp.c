...
{ // main
    ...
    int checkarguments = CheckArguments(argc, "ebcComp");
    ...
    EBCInitBinaryMagicNum(image1, inputFile1);

    // checking against the casted value due to endienness.
    if (EBCCheckMagicNumber(image1, argv[1]) == BAD_MAGIC_NUMBER)
    {
    ...
    int returned = EBCInitBinaryArray(image1, inputFile1, argv[1]);
    if (returned != 100)
    {
        return returned;
    }
    ...
    // initialise the magic number
    EBCInitBinaryMagicNum(image2, inputFile2);

    // checking against the casted value due to endienness.
    if (EBCCheckMagicNumber(image2, argv[2]) == BAD_MAGIC_NUMBER)
    {
        return BAD_MAGIC_NUMBER;
    }
    ...
    // initialise the array
    EBCInitBinaryArray(image2, inputFile2, argv[2]);
